...
# Some plots are drawn in order to show you the outliers
...
# in order to be easily removed.
# The cases are removed based on cook distances.
# The rest plots provide information only on the quality of the dataset
...
# Depends on Base R / Stats package only
...
# df_data_wt_id:  Must be a dataframe with ID variable
...
#              -  Thus cut_off = 10 will exclude only one value from above example
#                       - more than 10% difference must exist in order to be excluded
#              -  Thus cut_off = 1  will exclude two values from above example
#                       - more than 1% difference must exist in order to be excluded
...
### NOTE: Because of percentaging the distances, a value can have a 10 perc distance
#         point in the middle of the dataset this value can be removed from the dataset, 
#         if the cut-off / sensitivity can "catch" it e.g. "100 90 60__20 1 0"
...
#         eitherway both pairs of values will be reported
...
#         "detete.na" function is applied to remove any Row that includes "NA"s

### NOTE: It returns the dataset - check if your dataset is the same with the returned one
#         - e.g. Rows with "NA/s" may have been deleted. 
...
outlier_remove <- function( df_data_wt_id, id_name_var, cook_mahal, sensitivity,  cut_off, plotit  ) {
  ...
  ck_perc_0      <- round( ( ord_cooks / max( ord_cooks, na.rm=T ) )*100, 2 )
  ck_diff_dist   <- ck_perc_0 - c( ck_perc_0[-1], 0 )
  
  mh_perc_0      <- round( ( ord_mahal / max( ord_mahal, na.rm=T ) )*100, 2 )
...
# outlier_remove( ex_df, "a", "C", 0, 5, "T" )  # Zero points are removed, 
# outlier_remove( ex_df, "a", "C", 3, 1, "T" )  # 3 (*2) pairs of outliers are removed
