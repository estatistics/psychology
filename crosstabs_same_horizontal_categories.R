...
csv_files  = lapply(dirs_tmp, function(x) read.csv(x,  sep = "," ) )
...
for ( d in 1:length( csv_files ) ) {
  ...
  crosstbs$id <- 1:dim( crosstbs )
  
  tbl_st  <- crosstbs[crosstbs$id[ grepl(x=crosstbs[[1]], "total") ] + 2, ] 
  tbl_fn  <- crosstbs[crosstbs$id[ grepl(x=crosstbs[[1]], "Total") ], ]
  test_nm <- crosstbs[crosstbs$id[ grepl(x=crosstbs[[1]], "/TABLES=") ], ]
  cq_ln<- crosstbs[crosstbs$id[ grepl(x=crosstbs[[1]], "-Square") ], ]
  
  st_pos  <- unlist((gregexpr(pattern ="BY", test_nm[[1]])))
  fn_pos  <- unlist((gregexpr(pattern =" /FORMAT", test_nm[[1]])))
...
pos_st   <- rbind_null_df_lists( lapply(1:lgth_list, function(a) which( as.matrix( rbind_cross[[a]][1,] ) %in% lbls_unique )  ) )  
pos_orig <- rbind_null_df_lists( lapply(1:lgth_list, function(b) which( lbls_unique %in% rlist_labels[[b]] )  ) ) 
...
dt_transfer  <- rbind_cross[[i]]
dt_transfer[ na.omit(pos_orig[i,]) ] <- rbind_cross[[i]][ na.omit(pos_st[i,]) ]
dt_transfer[  seq_max[seq_max %!in% na.omit(pos_orig[i,]) ]  ] <- ""

rdt_alignd[[i]] <- dt_transfer
